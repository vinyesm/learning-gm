function [Z L S  ActiveSet hist param flag output] = cgan_l1_omega_03(inputData,param,startingZ,ActiveSet)
        ...
        if val<param.lambda*(1+eps_add)
            ...
            if pm && ActiveSet.atom_count>0
                ...
                if param.f==1
                    ...
                    [L,ActiveSet]=prox_cleaning(S,L,Sigma,ActiveSet,param,20,0);
                end
                Z=S+L;
                if param.f==1
...
if pp==1
    if ~isempty(ActiveSet.atoms)
        fprintf('Cleaning.. \n');
        if param.f==1
            [L,ActiveSet]=prox_cleaning_prox(S,L,inputData.Y,ActiveSet,param,100,1);
            Z=Z1+L;
        else
            if param.f==4
                Sigma=inputData.X1*inputData.X1;
            ...
            [L,ActiveSet]=prox_cleaning(S,L,Sigma,ActiveSet,param,100,1);
            Z=S+L;
        end
    end
end
...
if pt==1
    if ~isempty(ActiveSet.atoms)
        ...
        thresh=1e-6;
        [L,ActiveSet]=postprocessing(ActiveSet, thresh);
        Z=S+L;
    end
end
