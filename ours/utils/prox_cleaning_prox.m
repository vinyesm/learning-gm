function  [Z2,ActiveSet]=prox_cleaning_prox(Z1,Z2,Y,ActiveSet,param,T,debug)
...
L= norm(ActiveSet.atoms(:,1:ActiveSet.atom_count)'*ActiveSet.atoms(:,1:ActiveSet.atom_count),'fro')^2;%lipshitz
...
for t=1:T
    ...
    for i=order
        if debug
            ...
            obj(count)=.5*norm((Z1+Z)-Y,'fro')^2 + lam*reg;
            count=count+1;
        end
