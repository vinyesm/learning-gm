...
In this section we study the decomposability problem. The following concepts and theorems are a generalization of the sparse + low rank matrix decomposition problem studied in \citet{chandrasekaran2011rank}. Let $\D$ stand for $\RR^{p}$ or $\RR^{p_{1} \times p_{2}}$ and let $x_{1}^*,x_{2}^*,..,x_{m}^*\in \D$. Assume each $x_{i}^*$ encodes some specific structure.\TODO{ tell in better way} Given $y=\sum_{i=1}^{m}x_{i}^*$ , can we uniquely recover the decomposition $(x_{1}^*,x_{2}^*,..,x_{m}^*)$ ? \\

\GO{Maybe we should use the expression \emph{demixing} used in signal processing, rather than decomposition, or at least warn the reader that the fact that we are considering decomposable norms is because the main norms inducing parcimony are decomposable but that this is unrelated to the decomposability of the problem}.

In general solving the decomposition problem is intractable \TODO{explain}. Hence, we consider a tractable approach employing convex relaxations. We formulate a convex optimization problem for decomposition using a combination of \emph{decomposable} norms $\omega_{i}:\D\mapsto \RR^{+}$ as surrogates inducing the desired structure on the $i$-th component. 


\NEW{A first definition of a concept of \emph{decomposable norms} was introduced in \citet{negahban2012unified}. Related concepts with stronger ground in convex analysis were then proposed in \citet{candes2013simple} and extended to gauges in \citet{vaiter2015model} \citep[see also][]{fadili2013stable,vaiter2015low}.}

Following some of the ideas of~\citet{vaiter2015model}, we introduce a new definition.

\begin{mydef}
We say that the gauge $\omega$ has an \emph{orthogonally decomposable subgradient} at $x \in \RR^p$ if the projection of the origin $0$ on the affine span of $\partial \omega(x)$ is in the relative interior of $\partial \omega(x)$.
\end{mydef}
\GO{We probably do not need the relative interior condition. TODO: double check why \citet{vaiter2015model} impose that condition. This would be a problem when working with gauges defined on cones that are direct sums of a points and a cone because then $q_x$ is not in the relative interior of the cone...}
...
Natural examples of gauge with o.d.s.\ are provided by \emph{separable} norms~\citep[called \emph{decomposable} in][]{negahban2012unified}.
...
Note that the o.d.s.\ could possibly be generalized by replacing the orthogonal projection in the definition by oblique projection. 



\GO{Need to revisit the examples below based on the new definition.}
...
\end{align}
where $\gamma_{i}>0$ are  user-defined regularization parameters. With no further assumptions, the decomposability problem (\ref{eq:gen_opt}) is ill posed. However, under some incoherence conditions on tangent spaces we prove that the decomposition is exactly recovered. Proposition \ref{prop:genunicity} states existence and unicity \GO{uniqueness is better} of the solution of (\ref{eq:gen_opt}) and theorem \ref{theo:gentwo} states simple conditions that guarantee exact decomposition for problem  (\ref{eq:gen_opt}). \TODO{say that we can generalize to gauges}
%However, under some incoherence conditions on tangent spaces we prove that the decomposition is exactly recovered. In order to characterize incoherence conditions, we will remind the concepts of incoherence and tangent spaces.  We will extend the notion of incoherence to collections of subspaces.
...
Suppose that $y=\sum_{i=1}^m x_{i}^*$. \TODO{Introduce tangent spaces $\T_i $}.If
\BET
\item[(i)] $\forall i, \: \T_i \cap \text{span}\big ( (\T_j)_{j\neq i}\big) =\{0\}$
...
\item[(iii)] there exists a dual $q$ such that : $\forall i\in 1..m$, $P_i q=q_i^*$ and $P_i^{\perp} q < \gamma_i$
\EET
Then $\big(x_{i}^*\big)_{i=1..m}$ is the unique optimizer of problem (\ref{eq:gen_opt}).
\end{prop}
\begin{proof}
We start by showing that $\big(x_{i}^*\big)_{i=1..m}$ is an optimum before proving uniqueness. \TODO{$\big(x_{i}^*\big)_{i=1..m}$ is an optimum.}

Suppose there is another solution $\big(\hat{x}_{i}\big)_{i=1..m}=\big({x}_{i}^*+n_i\big)_{i=1..m}$. Then  $\big({x}_{i}^*+n_i\big)_{i=1..m}$ is also a minimizer . From equality constraints we must have $\sum_{i=1}^{m}n_i=0$ since $\sum_{i=1}^{m}{x}_{i}^*=y=\sum_{i=1}^{m}{x}_{i}^*+n_i$. Applying subgradient property at $\big(\hat{x}_{i}\big)_{i=1..m}$, we have that for any subgradient $(q_1,...,q_m)$,
\begin{align*}
...
\end{align*}
where in the first equality we use the fact that $\sum_{i=1}^{m} n_i=0$. We can select any subgradient $(q_1,...,q_m)$. \TODO{Suppose we can choose $q_i$ such that $\omega_i^{\circ}(P_i^{\perp}q_i)= \gamma_i$ and  $\left\langle P_i^{\perp}q_i,n_i\right\rangle = \gamma_i\omega_i(P_i^{\perp}n_i)$ }
Then,
\begin{align}
\label{unineq}
\sum_{i=1}^{m}\left\langle q_i,n_i\right\rangle \geq\sum_{i=1}^{m}\big(\gamma_i-\omega_i^{\circ}(P_i^{\perp}q)\big)\, \omega_i(P_i^{\perp}n_i),
\end{align}
where we used the inequality $\left\langle P_i^{\perp}q,P_i^{\perp}n_i\right\rangle\leq \omega_i^{\circ}(P_i^{\perp}q)\omega_i(P_i^{\perp}n_i)$. By assumption $\gamma_i-\omega_i^{\circ}(P_i^{\perp}q)>0$ for all $i=1..m$, so $\sum_{i=1}^{m}\left\langle q_i,n_i\right\rangle$ is strictly positive unless $\omega_i(P_i^{\perp}n_i)=0$ for all $i=1..m$. 
\TODO{unfinished proof}
\NEW{But $\omega_i(P_i^{\perp}n_i)=0$ implies that $P_i^{\perp} n_i=0$, which entails that $n_i \in \T_i$ for all $i$. Finally since $\sum_{i=1}^m n_i=0$, we have that $n_i=-\sum_{j \neq i} n_j$ so that $n_i \in \T_i \cap \text{span}\big ( (\T_j)_{j\neq i}\big)=\{0\}$ by assumption $(i)$. So $n_i=0$ for all $i$ and this proves the uniqueness of the decomposition.}
\end{proof}
...
\item[(ii)]  $\alpha>0$ %\TODO{better condition = lemma 5 :  $\T_i(x_{i}^*)\cap span\big((\T_j(x_{j}^*)_{j\neq i})\big)=\left\lbrace{0}\right\rbrace$}
\item[(iii)] $\frac{m(1-\alpha)}{\alpha}+ (1-\tilde{\alpha})<1$, %\TODO{better condition $\frac{m}{\alpha}\sum_{j\neq i}\xxi_{ij} + \sum_{j\neq i}\tilde{\xxi}_{ij}<1$}
\EET
...
\begin{proof}
The optimization problem \ref{eq:gen_opt} admits $\big(x_{i}^*\big)_{i=1..m}$ as a solution if and only if it satisfies first order gradient condition in the sense that there exists $q\in\D$ such that
$$
...
\item[(iii)]  
\begin{align*}
\omega_{i}^{\circ}(P_i^{\perp} q) = \omega_{i}^{\circ}(q-q_i^*) = \omega_{i}^{\circ}\big(\sum_{j=1}^m\epsilon_j + \sum_{j\neq i}^m q_j^*\big) 
