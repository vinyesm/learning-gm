...
Natural examples of are the trace norm for inducing low rank structure and $\ell_1$ norm for inducing spasity \TODO{detail spaces $\mathcal{M}$ and $\mathcal{M}^{\perp}$ ?}.\\
...
\end{align}
where $\gamma_{i}>0$ are  user-defined regularization paramateters. With no further assumptions, the decomposability problem (\ref{eq:gen_opt}) is ill posed. However, under some incoherence conditions on tangent spaces we prove that the decomposition is exactly recovered. Proposition \ref{prop:genunicity} states existence and unicity of the solution of (\ref{eq:gen_opt}) and theorem \ref{theo:gentwo} states simple conditions that guarantee exact decomposition for problem  (\ref{eq:gen_opt}). \TODO{say that we can generalize to gauges}
...
Suppose the $y=\big(x_{i}^*\big)_{i=1..m}$. \TODO{Introduce tangent spaces $\T_i $}.If
\BET
...
\end{prop}
\begin{proof}
We start by showing that $\big(x_{i}^*\big)_{i=1..m}$ is an optimum before prooving unicity. \TODO{$\big(x_{i}^*\big)_{i=1..m}$ is an optimum.}
...
\end{align*}
Decomposing the subradients on $\T_i$ and $\T_i^{\perp}$, gives $q_i=q_i^* + P_i^{\perp}(q_i)$ and  $q=q_i^* + P_i^{\perp}(q)$. Using this decompositions we have,
\begin{align*}
...
&= \left\langle P_i^{\perp}(q)+ P_i^{\perp}(q_i),n_i\right\rangle + \left\langle q,n_i\right\rangle
\end{align*}
Thus,
\begin{align*}
\sum_{i=1}^{m}\left\langle q_i,n_i\right\rangle &=\sum_{i=1}^{m}\left\langle P_i^{\perp}q+ P_i^{\perp}q_i,n_i\right\rangle \\
&=\sum_{i=1}^{m}\left\langle P_i^{\perp}q+ P_i^{\perp}q_i,P_i^{\perp}n_i\right\rangle \\
\end{align*}
...
\begin{align}
\label{unineq}
\sum_{i=1}^{m}\left\langle q_i,n_i\right\rangle \leq\sum_{i=1}^{m}\big(\gamma_i-\omega_i^{\circ}(P_i^{\perp}q)\big)\omega_i(P_i^{\perp}n_i)
\end{align}
where we used the inequality $\left\langle P_i^{\perp}q,P_i^{\perp}n_i\right\rangle\leq \omega_i^{\circ}(P_i^{\perp}q)\omega_i(P_i^{\perp}n_i)$. By assumption $\gamma_i-\omega_i^{\circ}>0$ for all $i=1..m$, so $\sum_{i=1}^{m}\left\langle q_i,n_i\right\rangle$ is strictly positive unless $\omega_i(P_i^{\perp}n_i)=0$. 
