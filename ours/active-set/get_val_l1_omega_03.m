function [obj, loss, pen] = get_val_l1_omega_03(L,S,inputData,param,ActiveSet)
...
if ActiveSet.atom_count>0
    pen = param.lambda*dot(cf,ActiveSet.alpha)+param.mu*(sum(abs(S(:))));
else
    pen=param.mu*(sum(abs(S(:))));
end
